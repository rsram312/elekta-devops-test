...
7. **Credentials.** Username `Elekta` / password `ElektaDevopsTest123!` as specified by the assessment. The password is never committed - supplied as `TF_VAR_admin_password` via a gitignored `terraform.tfvars` locally or a secret variable group in Azure DevOps.
